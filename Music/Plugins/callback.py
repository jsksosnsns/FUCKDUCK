from pyogram import Client, filters
 ...
 from Music import aiohttpssession as session
 ...
 """,
     ...
     if await is_active_chat(chat_id):
         if await is_music_playing(chat_id):
             await CallbackQuery.answer(
                 "I don't think if something is paused in voice chat",
                 show_alert=True,
             )
             return
         else:
             ...
             await CallbackQuery.answer("Continue", show_alert=True)
             user_id = CallbackQuery.from_user.id
             user_name = CallbackQuery.from_user.first_name
             rpk = "[" + user_name + "](tg://user?id=" + str(user_id) + ")"
             await CallbackQuery.message.reply(
 ...
 **Skip Button Used By** {rpk}
 ...
 """
             ...
             await CallbackQuery.answer("Voice Chat Skipped", show_alert=True)
 ...
 **Reason**:{e}
 ...
 async def stopvc(_, CallbackQuery):
     ...
     if await is_active_chat(chat_id):
         ...
         try:
             ...
             pass
         await remove_active_chat(chat_id)
         await CallbackQuery.answer("Terminated", show_alert=True)
         user_id = CallbackQuery.from_user.id
         user_name = CallbackQuery.from_user.first_name
         rpk = "[" + user_name + "](tg://user?id=" + str(user_id) + ")"
 ...
 async def group_playlist(_,CallbackQuery):
     ...
     await save_playlist(chat_id, videoid, assist)
     Name = CallbackQuery.from_user.first_name
 ...
 async def pla_playylistt(_,CallbackQuery):
     ...
     await save_playlist(userid, videoid, assist)
